        ...
        <div className="lg:hidden bg-white px-4 py-6 shadow-md absolute w-full z-40">
          <ul className="space-y-4">
            {navLinks.map((link) => (
              <li key={link.name}>
                <div>
                  {/* Title opens link + submenu */}
                  <Link
                    href={link.href || "#"}
                    onClick={() => setOpenMobileSubMenu(link.name)}
                    className="w-full text-left font-semibold text-black text-lg block"
                  >
                    {link.name}
                  </Link>
                          ...
                          ? (link as ProductMenu).subItems?.map((item) => (
                              <li key={item.title}>
                                ...
                                <Link
                                  href={item.href}
                                  onClick={() =>
                                    setOpenMobileSubSubMenu(item.title)
                                  }
                                  className="text-left text-black font-medium block"
                                >
                                  {item.title}
                                </Link>
