...
//                   openMobileSubSubMenu === item.title ? null : item.title
//                 )
//               }
//             >
//               {item.title}
//               <span>{openMobileSubSubMenu === item.title ? "-" : "+"}</span>
//             </button>
...
//           </>
//         ) : (
//           // Render direct link if no subMenu (EPC Solutions)
//           <Link
//             href={item.href}
//             onClick={() => setMobileMenuOpen(false)}
...
{mobileMenuOpen && (
  ...
  ? (link as ProductMenu).subItems?.map((item) => (
      <li key={item.title} className="flex flex-col">
        ...
        {/* Sub-menu items: each is clickable */}
        {openMobileSubSubMenu === item.title && item.subMenu && (
          <ul className="ml-4 mt-1 space-y-1">
            {item.subMenu.map((sub) => (
              <li key={sub.name}>
                <Link
                  href={sub.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-sm text-black py-1 block"
                >
                  {sub.name}
                </Link>
              </li>
