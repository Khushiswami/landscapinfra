...
export default function BannerP() {
  ...
  return (
    <section className="relative w-full h-[94vh] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col justify-center h-full max-w-6xl mx-auto px-6">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            transition={{ duration: 0.7 }}
          >
            <h1 className="text-2xl sm:text-4xl md:text-6xl font-bold text-white mb-4 sm:mb-6 drop-shadow-lg">
              {slides[active].title}
            </h1>
            <p className="text-sm sm:text-lg md:text-xl text-gray-200 mb-6 sm:mb-8 max-w-xl leading-relaxed">
              {slides[active].desc}
            </p>

            <button
              onClick={() => router.push(slides[active].link)}
              className="px-6 sm:px-8 py-2 sm:py-3 bg-white text-blue-700 font-semibold rounded-full shadow-lg hover:bg-blue-100 transition flex items-center gap-2 text-sm sm:text-base"
            >
      ...
      {/* Bottom Navigation Progress + Controls */}
      <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 w-full px-4 sm:px-6 z-10">
        <div className="flex flex-wrap justify-center items-end gap-4 sm:gap-8 relative">
          {slides.map((s, i) => (
            <div
              key={i}
              onClick={() => setActive(i)}
              className="cursor-pointer w-32 sm:w-52"
            >
              {/* Progress Bar */}
              ...
              <p
                className={`text-[10px] sm:text-xs font-medium leading-snug text-center ${
                  active === i ? "text-white" : "text-gray-300"
                }`}
              >
                {s.title}
          ...
          {/* Play / Pause Button */}
          <div className="absolute -top-12 right-0 sm:right-4">
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full border border-white/50 hover:bg-white/10 transition"
            >
              {isPlaying ? (
